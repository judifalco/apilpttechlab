...
// void llenarArray(string arrayUsuario[], int tamanoArray){ 
// //Funcion para que en el caso de acertar antes de usar todos los intentos
// //los indices del array no queden con valores basura
//     for (int i=0; i<tamanoArray;i++){
//         arrayUsuario[i] = "No utilizaste el intento numero";
//     }
// }
//No es necesaria ya que en el caso de no completar los intentos
//Se muestra solo hasta el ultimo


//Función para imprimir por pantalla un array
void recorridaArray(string array[], int tamanoArray){
    cout << "[ ";
    for(int i=0;i<tamanoArray;i++) {
...
bool encontradoEnArray(const string arrayUsuario[], int tamanoArray, int numeroUsuario){
    string stringUsuario = to_string(numeroUsuario);
    for (int i=0; i<tamanoArray;i++){
        //Si el array tiene 8 elementos recorrerá hasta el [7] (o sea array ompleto)
        if (arrayUsuario[i] == stringUsuario ) {
            return true;
...
int main(){
    ...
    //y los llenamos por si algun indice no se usa
    string array_usuario[intentos_maximos];
    ...
    while (i < intentos_maximos) { //Ya que i arranca en 0 elegimos que sea menor estricto del maximo de intentos
        ...
        array_usuario[i] = to_string(numero_usuario); //Añadimos al array el numero del usuario en forma string
